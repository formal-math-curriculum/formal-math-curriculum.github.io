...
async function fixture(deploymentAuthorized) {
  const root = await mkdtemp(join(tmpdir(), 'm5-5-release-gate-'));
  await mkdir(join(root, 'validation'), { recursive: true });
  await mkdir(join(root, 'dist', 'validation', 'm5-5'), { recursive: true });
  await writeFile(join(root, 'validation', 'm5-5-current.json'), JSON.stringify({
  ...
  await writeFile(
    ...
    '<urlset><url><loc>https://formal-math-curriculum.github.io/</loc></url><url><loc>https://formal-math-curriculum.github.io/validation/m5-5/</loc></url><url><loc>https://formal-math-curriculum.github.io/validation/m5-5/collision/</loc></url></urlset>'
  );
  return root;
}

test('MAT-399 F02 deployment fails closed while the current record denies authorization', async () => {
...
test('MAT-399 F02 an authorized release removes the synthetic route and sitemap entry', async () => {
  ...
  assert.doesNotMatch(sitemap, /validation\/m5-5/u);
  assert.deepEqual(provenance.syntheticRoutesRemoved, ['/validation/m5-5/']);
});

test('MAT-399 F02 release selector and record cannot escape validation authority', async () => {
  const root = await fixture(true);
  await writeFile(join(root, 'validation', 'm5-5-current.json'), JSON.stringify({
    schemaVersion: 'p5-current-baseline-selector/v1',
