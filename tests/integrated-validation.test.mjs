...
test('M5.10 release workflow advances from exact authority through durable draft and Pages upload', async () => {
  ...
  assert.match(workflow, /upload_url[\s\S]*releases\/assets\/\$asset_id[\s\S]*--data-binary "@release-assets\/\$asset"[\s\S]*Accept: application\/octet-stream[\s\S]*cmp/u);
  assert.doesNotMatch(workflow, /--hostname uploads\.github\.com/u);
