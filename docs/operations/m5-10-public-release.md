...
Before deployment, the workflow binds the release tag directly to the exact workflow commit and refuses a published release. A retry after a partial run may rebind the tag only when the API identifies exactly one matching draft; a published tag is never moved. Recovery addresses the draft by release ID, replaces every staged asset through the release's API-provided upload relation using raw binary requests, downloads the draft again and byte-compares all six files before deployment. Publication also addresses that unique draft by ID, verifies the tag and draft state before transition, then confirms both the published state and unchanged tag target.
