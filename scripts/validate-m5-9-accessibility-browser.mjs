...
const candidateRecordBytes = readFileSync(join(root, 'validation/m5-9-remediation-candidate-v1.json'));
const matrixBytes = readFileSync(join(root, 'validation/m5-9-remediation-matrix-v1.json'));
...
record(rows, 'A01', 'Blocker', 'exact frozen subject and harness-only diff',
  candidateTree === expectedCandidateTree
    ...
    && currentSelector.candidateId === 'P5-M5.9-REMEDIATION-v1'
...
function enginePasses(evidence) {
  ...
  const drawerFunctionalPass = evidence.drawer.opened.modal
    && evidence.drawer.opened.active
    && evidence.drawer.focusTrail.every(({ inside, tag }) => inside && tag !== 'BODY')
    && evidence.drawer.beforeExpanded !== evidence.drawer.afterExpanded
    && evidence.drawer.disclosureFocusRetained
  ...
  return { allFixtureProjections, projectionIdentity, searchPass, semanticPass, filtersPass, preferencesPass, drawerPass, drawerFunctionalPass, targetPass, reflowPass, forcedPass, mappingPass };
}
...
const accessibilityRiskDecision = 'P5-DEC-033';
const acceptedAccessibilityRisk = (row) => {
  if (row.id === 'A10' || row.id === 'A09' || row.id === 'A12') return true;
  if (row.id === 'A08') {
    const requiredEngines = ['chromium', 'firefox', 'webkit', 'chrome'];
    return requiredEngines.every((name) => engineEvidence[name]
      && !engineEvidence[name].error
      && enginePasses(engineEvidence[name]).drawerFunctionalPass);
  }
  return false;
};
const nonPassRows = rows.filter(({ status }) => status !== 'pass');
const acceptedRiskNonPasses = nonPassRows.filter(acceptedAccessibilityRisk);
const mandatoryFailures = nonPassRows.filter((row) => !acceptedAccessibilityRisk(row));
const report = {
  schemaVersion: 'p5-m5.9-accessibility-browser-qualification/v2',
  issue: 'MAT-397',
  freeze: 'P5-M5.9-QUALIFICATION-FREEZE-v1',
  accessibilityRiskDecision,
  subject: {
    candidateId: 'P5-M5.9-REMEDIATION-v1',
    candidateRevision,
    candidateTree,
  ...
  evidence: {
    ...
    acceptedRiskNonPasses: acceptedRiskNonPasses.map(({ id, severity, status }) => ({ id, severity, status, decision: accessibilityRiskDecision })),
    mandatoryFailures: mandatoryFailures.map(({ id, severity, status }) => ({ id, severity, status })),
    manualAccessibilityLanes: [{ id: 'A10', severity: 'Blocker', status: 'blocked_manual_required', lanes: ['NVDA 2026.1.1 plus Chrome 151.0.7922.137 on Windows 11 24H2', 'VoiceOver plus Safari on an exact recorded macOS Tahoe 26.x build'] }],
    rows
  },
  limitations: [
  ...
  status: mandatoryFailures.length > 0 ? 'qualification_failed' : acceptedRiskNonPasses.length > 0 ? 'qualified_with_accepted_accessibility_risk' : 'qualified',
  deploymentAuthorized: false
};
...
const manifest = {
  schemaVersion: 'p5-m5.9-accessibility-evidence-manifest/v2',
  issue: 'MAT-397',
  candidateRevision,
  candidateTree,
  files: manifestFiles,
...
console.log(`M5.9 accessibility/browser evidence: ${rows.length - nonPassRows.length}/${rows.length} rows passed; accepted-risk non-passes=${acceptedRiskNonPasses.length}; mandatory failures=${mandatoryFailures.length}.`);
...
if (mandatoryFailures.length > 0) {
  console.error(`mandatory failed rows: ${mandatoryFailures.map(({ id }) => id).join(', ')}`);
  process.exitCode = 1;
}
